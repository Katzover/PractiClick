...
3. **(Optional) Use `controlpanel1.html`** for room management (admin only).
...
3. **(אופציונלי) השתמש ב-`controlpanel1.html`** לניהול חדרים (למנהלים בלבד).
4. **אין צורך בהתקנה** – הכל עובד בדפדפן.
