...
function renderLogs() {
    ...
    logs.slice().reverse().forEach((log, index) => {
        ...
        item.innerHTML = `
            ...
            ${log.note ? `<span style="color:#60aaff; font-size: 0.9em;">Note: ${log.note}</span>` : ''}
        `;
...
async function exportWeeklySummary() {
    const { error } = await supabase
        ...
        .eq('name', `${localStorage.getItem('UserName')} - ${toString(getWeekStart())}`)
    if (error) {
...
async function establishUserdata() {
    ...
    await supabase
        .from('summaries')
        .insert({ name: `${localStorage.getItem('UserName')} - ${toString(getWeekStart())}`, json: getWeeklySummaryJson() });
